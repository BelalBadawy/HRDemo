...
HrDemo.Domain          <-- no dependenciesHrDemo.Application      <-- depends on Domain onlyHrDemo.Infrastructure    <-- depends on Application (+ Domain transitively)HrDemo.API                <-- depends on Application + Infrastructure
...
If a user request conflicts with anything in this file, point out the conflict and ask before proceeding rather than silently picking one side.